...
func ReplaceTransaction(db *gorm.DB, transaction models.Transaction) error {
	err := daos.ReplaceTransaction(db, uint64(transaction.ID), copyTransactionDTOProperties(transaction))
	if err != nil {
		return echo.NewHTTPError(http.StatusBadRequest, err.Error())
	}
	return nil
