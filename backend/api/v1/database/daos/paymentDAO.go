...
func GetUserPaymentById(db *gorm.DB, transactiondtoid uint64, id uint64) (dtos.PaymentDTO, error) {
	paymentDTO := dtos.PaymentDTO{}
	if err := db.Where("id = ? AND transaction_dto_id = ?", id, transactiondtoid).First(&paymentDTO).Error; err != nil {
		return dtos.PaymentDTO{}, nil
	}
	return paymentDTO, nil
}
