...
func authLogout(c echo.Context) error {
	...
	if profile != "DEV" {
		c.SetCookie(&http.Cookie{Name: "userinfo", Value: "", HttpOnly: true, SameSite: http.SameSiteNoneMode, Path: "/", Secure: true, Expires: time.Unix(0, 0)})
	}
...
func authWithOAuthCallback(c echo.Context) error {
	...
	if c.FormValue("state") != oauthState.Value {
		...
		c.SetCookie(&http.Cookie{Name: "login_state", Value: "failure", Path: referer.Path})
	...
	c.SetCookie(utils.GetTokenCookie(t))
	c.SetCookie(&http.Cookie{Name: "oauthstate", Value: "", Expires: time.Unix(0, 0)})
	c.SetCookie(&http.Cookie{Name: "login_state", Value: "success", Path: referer.Path})
	return c.Redirect(http.StatusPermanentRedirect, redirect.(string))
}
